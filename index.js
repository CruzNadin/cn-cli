...
function ModuleCreate(projectFolder, appname) {
  ...
  try {
    fs.readFile(
      ...
      (error, data) => {
        ...
        fs.writeFileSync(projectFolder + "/android/build.gradle", settingsdata);
      }
    );
    ...
    fs.readFile(
      ...
      (error, data) => {
        ...
        var replacedata = settingsdata.replace("tulparyazilim_base", appname);
        fs.writeFileSync(
  ...
  try {
    ...
    console.error(
      "There was an error while deleting base project folder. Error: " + error
    );
  }
  ...
  try {
    shell.cd(path);
    shell.exec("git clone " + tulpar_base_url);
  } catch (error) {
    console.error(
      "There was an error while fetching github project. Error: " + error
    );
  }
